...
start_aliddns(){
	...
	sh /jffs/softcenter/scripts/aliddns_update.sh >/dev/null 2>&1

	# check startup file
	if [ ! -L "/jffs/softcenter/init.d/S98Aliddns.sh" ]; then 
		ln -sf /jffs/softcenter/scripts/aliddns_config.sh /jffs/softcenter/init.d/S98Aliddns.sh
	fi
}
...
start)
	if [ "$aliddns_enable" == "1" ]; then
		...
		echo_date "======================= Aliddns - WAN拨号触发启动 ========================" > $LOG_FILE
		start_aliddns > $LOG_FILE
	else
		logger "[软件中心]: 阿里DDNS未设置开机启动，跳过！"
	fi
	;;
stop)
	stop_aliddns > $LOG_FILE
	;;
restart)
	# submit
	if [ "$aliddns_enable" == "1" ]; then
		echo_date "========================= Aliddns - 手动启动 ========================" > $LOG_FILE
		start_aliddns > $LOG_FILE
	else
		echo_date "========================= Aliddns - 关闭 ===========================" > $LOG_FILE
		stop_aliddns > $LOG_FILE
	fi
	;;
clean)
