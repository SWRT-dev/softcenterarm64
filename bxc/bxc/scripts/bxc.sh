...
BXC_CONF="/jffs/softcenter/bxc/bxc.config"
...
vpn_env(){
	...
	if [ ! -s $BXC_SSL_CA ];then
		if [ -s /jffs/softcenter/bxc/ca.crt ];then
			logdebug "/jffs/softcenter/bxc/ca.crt exist, copy to $BXC_SSL_CA"
			mkdir -p /jffs/bcloud/
			cp -f /jffs/softcenter/bxc/ca.crt $BXC_SSL_CA > /dev/null 2>&1
		else
	...
	if [ ! -s $BXC_SSL_CRT ];then
		if [ -s /jffs/softcenter/bxc/client.crt ];then
			logdebug "/jffs/softcenter/bxc/client.crt exist, copy to $BXC_SSL_CRT"
			mkdir -p /jffs/bcloud/
			cp -f /jffs/softcenter/bxc/client.crt $BXC_SSL_CRT > /dev/null 2>&1
		else
	...
	if [ ! -s $BXC_SSL_KEY ];then
		if [ -s /jffs/softcenter/bxc/client.key ];then
			logdebug "/jffs/softcenter/bxc/client.key exist, copy to $BXC_SSL_KEY"
			mkdir -p /jffs/bcloud/
			cp -f /jffs/softcenter/bxc/client.key $BXC_SSL_KEY > /dev/null 2>&1
		else
...
start_bxc(){
	...
	if [ $worker_status -ne 0 ];then
		...
		port_used=`netstat -lan | grep "LISTEN" | grep ":$BXC_WORKER_PORT" > /dev/null 2>&1; echo $?`
...
bound_bxc(){
	...
	curl -k -m 10 -H "Content-Type: application/json" -d "{\"email\":\"$email\", \"bcode\":\"$bcode\", \"mac_address\":\"$mac\"}" -w "\nstatus_code:"%{http_code}"\n" $BXC_BOUND_URL > /jffs/softcenter/bxc/curl.res
	logdebug "curl -k -m 10 -H \"Content-Type: application/json\" -d \"{\"email\":\"$email\", \"bcode\":\"$bcode\", \"mac_address\":\"$mac\"}\" -w \"\nstatus_code:\"%{http_code}\"\n\" $BXC_BOUND_URL"
	curl_code=`grep 'status_code' /jffs/softcenter/bxc/curl.res | awk -F: '{print $2}'`
	if [ -z $curl_code ];then
		dbus set bxc_bound_status="绑定请求超时，请确认网络正常后重试"
		logerr "Server has no response, exit"
		exit 1
	elif [ "$curl_code"x == "200"x ];then
		echo -e `cat /jffs/softcenter/bxc/curl.res | /jffs/softcenter/scripts/bxc-json.sh | egrep "\"Cert\",\"key\"" | awk -F\" '{print $6}' | sed 's/"//g'` | base64_decode > $BXC_SSL_KEY
		echo -e `cat /jffs/softcenter/bxc/curl.res | /jffs/softcenter/scripts/bxc-json.sh | egrep "\"Cert\",\"cert\"" | awk -F\" '{print $6}' | sed 's/"//g'` | base64_decode > $BXC_SSL_CRT
		echo -e `cat /jffs/softcenter/bxc/curl.res | /jffs/softcenter/scripts/bxc-json.sh | egrep "\"Cert\",\"ca\"" | awk -F\" '{print $6}' | sed 's/"//g'` | base64_decode > $BXC_SSL_CA
		if [ ! -s $BXC_SSL_KEY ];then
		...
		cat /jffs/softcenter/bxc/curl.res | /jffs/softcenter/scripts/bxc-json.sh | egrep '\["details"\]' > /dev/null
		if [ $? -eq 0 ];then
			fail_detail=`cat /jffs/softcenter/bxc/curl.res | /jffs/softcenter/scripts/bxc-json.sh | egrep '\["details"\]' | awk -F\" '{print $(NF-1)}'`
			dbus set bxc_bound_status="$fail_detail"
			logerr "bound failed with server response: $fail_detail"
			exit 1
		else
			dbus set bxc_bound_status="绑定请求超时，请确认网络正常后重试"
			logerr "Server response code: $curl_code, please check /jffs/softcenter/bxc/curl.res"
			exit 1
		fi
	fi

	# 备份绑定信息（邀请码 + 证书文件）
	cp -f $BXC_SSL_DIR/* /jffs/softcenter/bxc/
	echo $bcode > /jffs/softcenter/bxc/bcode
	echo $email > /jffs/softcenter/bxc/email

	# 清理临时文件
	# rm -f /jffs/softcenter/bxc/curl.res
}
booton_bxc(){
	# 开启开机自启动
	if [ ! -L "/jffs/softcenter/init.d/S97bxc.sh" ]; then 
        ln -sf /jffs/softcenter/scripts/bxc.sh /jffs/softcenter/init.d/S97bxc.sh
    fi
    [ ! -L "/jffs/softcenter/init.d/S97bxc.sh" ] && logerr "BxC-Node start onboot enable failed"
    dbus set bxc_onboot="yes"
}
bootoff_bxc(){
	# 关闭开机自启动
    rm -f /jffs/softcenter/init.d/S97bxc.sh
    [ -L "/jffs/softcenter/init.d/S97bxc.sh" ] && logerr "BxC-Node start onboot disable failed"
...
cron_add(){
	cron_exist=`cru l | grep "/jffs/softcenter/scripts/bxc-mon.sh" > /dev/null 2>&1;echo $?`
	if [ $cron_exist -ne 0 ];then
		cru a BxcMon "*/10 * * * * /jffs/softcenter/scripts/bxc-mon.sh > /dev/null 2>&1"
	fi
}
cron_del(){
	cron_exist=`cru l | grep "/jffs/softcenter/scripts/bxc-mon.sh" > /dev/null 2>&1;echo $?`
	if [ $cron_exist -eq 0 ];then
		cru d BxcMon "*/10 * * * * /jffs/softcenter/scripts/bxc-mon.sh > /dev/null 2>&1"
	fi
}
update_bxc(){
	stop_bxc
	...
	if [ -s $BXC_PKG ];then
		...
		cp -rf /tmp/bxc/install.sh /jffs/softcenter/scripts/bxc_install.sh
