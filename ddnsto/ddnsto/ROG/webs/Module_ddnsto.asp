...
function get_dbus_data() {
	$.ajax({
		type: "GET",
		url: "/dbconf?p=ddnsto_",
		dataType: "script",
		async: false,
		success: function(data) {
			db_ddnsto = db_ddnsto_;
...
function onSubmitCtrl() {
	...
	db_ddnsto["action_script"]="ddnsto_config.sh";
	db_ddnsto["action_mode"] = "restart";
	db_ddnsto["current_page"] = "Module_db_ddnsto.asp";
...
function menu_hook(title, tab) {
	tabtitle[tabtitle.length - 1] = new Array("", "软件中心", "离线安装", "ddnsto 远程控制");
...
function version_show() {
	$.ajax({
        url: 'https://sc.paldier.com/arm64/ddnsto/config.json.js',
		type: 'GET',
		dataType: 'jsonp',
