...
<style>
	...
	#fastd1ck_switch, #fastd1ck_status, #fastd1ck_settings, #fastd1ck_log, #fastd1ck_help {
		border:1px solid #91071f;
	}
...
function get_run_status(){
	$.ajax({
		type: "GET",
		url: "/dbconf?p=fastd1ck_",
		dataType: "script",
		async: false,
		success: function(data) {
			var fastd1ck_status = db_fastd1ck_;
...
function get_pid_status(){
	...
	var dbus = {};
	dbus["action_script"]="fastd1ck_status.sh";
	dbus["action_mode"] = " Refresh ";
	dbus["current_page"] = "Module_fastd1ck.asp";
	dbus["next_page"] = "Module_fastd1ck.asp";
	$.ajax({
		type: "POST",
		cache:false,
		url: "/applydb.cgi?p=fastd1ck",
		data: $.param(dbus),
		dataType: "text",
		success: function(response){
			//E("script_status").innerHTML = response.result;
			setTimeout("check_fastd1ck_status();", 10000);
...
function check_fastd1ck_status() {
			  $.ajax({
			    url: '/res/fastd1ck_status.html',
			    dataType: 'html',
			    error: function(xhr) {
			      setTimeout("check_fastd1ck_status();", 5000);
			    },
			    success: function(response) {
			      if (response.search("XU6J03M6") != -1) {
				fastd1ck_status = response.replace("XU6J03M6", " ");
				//alert(aria2_status);
				E("script_status").innerHTML = fastd1ck_status;
				return true;
			      }
...
function save() {
	...
	dbus_new["action_script"]="fastd1ck_config.sh";
	dbus_new["action_mode"] = "restart";
	dbus_new["current_page"] = "Module_fastd1ck.asp";
	dbus_new["next_page"] = "Module_fastd1ck.asp";
	$.ajax({
		url: "/applydb.cgi?p=fastd1ck",
		cache: false,
		type: "POST",
...
function menu_hook() {
	tabtitle[tabtitle.length - 1] = new Array("", "软件中心", "离线安装", "迅雷快鸟");
	tablink[tablink.length - 1] = new Array("", "Main_Soft_center.asp", "Main_Soft_setting.asp", "Module_fastd1ck.asp");
}
</script>
</head>
<body onload="init();">
...
<input type="hidden" name="current_page" value="Module_fastd1ck.asp"/>
<input type="hidden" name="next_page" value="Module_fastd1ck.asp"/>
												...
												<tr id="switch_tr">
													...
													<td colspan="2">
														...
														<div style="display:table-cell;float: left;margin-left:270px;margin-top:-32px;position: absolute;padding: 5.5px 0px;">
															<a type="button" class="ss_btn" target="_blank" href="https://github.com/paldier/softcenter/blob/master/fastd1ck/Changelog.txt">更新日志</a>
														</div>
													</td>
												</tr>
											</table>
											...
											<table style="margin:-1px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
												...
												<td>
													<ul>
														...
														<li>迅雷快鸟提速主脚本为在/jffs/softcenter/bin/fastd1ck_main.sh，有能力的可以自行调试修改。</li>
														<li>本插件的维护地址在<a href="https://github.com/paldier/softcenter" target="_blank" ><i><u>https://github.com/paldier/softcenter</u></i></a>，欢迎到此反馈问题！</li>
													</ul>
												</td>
												</tr>
											</table>
