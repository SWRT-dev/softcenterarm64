...
function get_status() {
    $j.ajax({
        url: 'logreaddb.cgi?p=frps_status.log&script=frps_status.sh',
        dataType: 'html',
        error: function(xhr) {
...
function onSubmitCtrl() { //提交操作，提交时运行config-frps.sh，显示5秒的载入画面
	...
	db_frps["action_script"]="frps-config.sh";
	db_frps["action_mode"] = "restart";
	$j.ajax({
		url: "/applydb.cgi?p=frps",
		cache: false,
		type: "POST",
		dataType: "text",
		data: $.param(db_frps),
