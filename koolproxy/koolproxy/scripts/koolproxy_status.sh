...
rules_date_local=`cat /jffs/softcenter/koolproxy/data/rules/koolproxy.txt  | sed -n '3p'|awk '{print $3,$4}'`
rules_nu_local=`grep -E -v "^!" /jffs/softcenter/koolproxy/data/rules/koolproxy.txt | wc -l`
video_date_local=`cat /jffs/softcenter/koolproxy/data/rules/koolproxy.txt  | sed -n '4p'|awk '{print $3,$4}'`
daily_nu_local=`grep -E -v "^!" /jffs/softcenter/koolproxy/data/rules/daily.txt | wc -l`
custom_nu_local=`grep -E -v "^!" /jffs/softcenter/koolproxy/data/rules/user.txt | wc -l`
...
do
	...
	if [ -f "/jffs/softcenter/koolproxy/data/rules/$tprule_name" ]; then
		eval tprule_nu_$i=`grep -E -v "^!" /jffs/softcenter/koolproxy/data/rules/$tprule_name | wc -l`
