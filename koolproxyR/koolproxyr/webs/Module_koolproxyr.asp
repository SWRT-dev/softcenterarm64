...
function get_dbus_data(){
	$.ajax({
	  	type: "GET",
		url: "/dbconf?p=koolproxyr_&v=<% uptime(); %>",
		dataType: "html",
		cache:false,
...
function generate_options(){
	for(var i = 0; i < 24; i++) {
		...
		$("#koolproxyr_reboot_inter_hour").val(12);
	}
	for(var i = 0; i < 60; i++) {
...
function conf2obj(){
	...
	var params_chk = ["koolproxyr_enable", "koolproxyr_rule_enable_d1", "koolproxyr_rule_enable_d2", "koolproxyr_rule_enable_d3", "koolproxyr_rule_enable_d4", "koolproxyr_rule_enable_d5", "koolproxyr_rule_enable_d6", "koolproxyr_rule_enable_d7", "koolproxyr_rule_enable_d8"];
...
function update_visibility(r){
	...
	if($(r).attr("id") == "koolproxyr_mode"){
		if(E("koolproxyr_mode").value == 3){
			...
			E("koolproxyr_rule_enable_d7").checked = false;
			E("koolproxyr_rule_enable_d8").checked = false;
			...
			E("koolproxyr_rule_enable_d7").checked = true;
			E("koolproxyr_rule_enable_d8").checked = true;
			for ( var i = 1; i <= maxrule; ++i ) {
				if (E("koolproxyr_rule_enable_" + i)){
...
function refresh_acl_table() {
	$.ajax({
		...
		success: function(response) {
			...
			if (typeof dbus["koolproxyr_acl_default_mode"] !== "undefined") {
				$('#koolproxyr_acl_default_mode').val(dbus["koolproxyr_acl_default_mode"]);
			} else {
				$('#koolproxyr_acl_default_mode').val("1");
			}
		}
	});
}
function refresh_acl_html() {
	...
	for (var field in acl_confs) {
		...
		code = code + '<tr>';
		code = code + '<td>';
		code = code + '<input type="text" placeholder="" id="koolproxy_acl_ip_' + ac["acl_node"] + '" name="koolproxyr_acl_ip_' + ac["acl_node"] + '" class="input_option_2" maxlength="50" style="width:140px;" value="' + ac["ip"] + '" />';
		code = code + '</td>';
		code = code + '<td>';
		code = code + '<input type="text" placeholder="" id="koolproxyr_acl_mac_' + ac["acl_node"] + '" name="koolproxyr_acl_mac_' + ac["acl_node"] + '" class="input_option_2" maxlength="50" style="width:140px;" value="' + ac["mac"] + '" />';
		code = code + '</td>';
		code = code + '<td>';
		code = code + '<input type="text" placeholder="" id="koolproxyr_acl_name_' + ac["acl_node"] + '" name="koolproxyr_acl_name_' + ac["acl_node"] + '" class="input_option_2" maxlength="50" style="width:140px;" placeholder="" />';
		code = code + '</td>';
		code = code + '<td>';
		code = code + '<select id="koolproxyr_acl_mode_' + ac["acl_node"] + '" name="koolproxyr_acl_mode_' + ac["acl_node"] + '" style="width:140px;margin:0px 0px 0px 2px;" class="input_option_2">';
	...
	code = code + '<td><font style="margin-left:8px">默认规则</font></td>';
	code = code + '<td>';
	code = code + '<select id="koolproxyr_acl_default_mode" name="koolproxyr_acl_default_mode" style="width:140px;margin:0px 0px 0px 2px;" class="input_option_2";">';
...
function refresh_rule_table() {
	$.ajax({
		...
		success: function(response) {
			$.globalEval(response);
			dbus = db_koolproxyr_;
			$("#rule_table").find("tr:gt(9):lt(-1)").remove();
			$('#rule_table tr:eq(9)').after(refresh_rule_html());
...
function refresh_rule_html() {
	...
	for (var field in rule_confs) {
		...
		code = code + '<input type="checkbox" id="koolproxy_rule_enable_' + ac["rule_node"] + '" name="koolproxyr_rule_enable_' + ac["rule_node"] + '" />';
		code = code + '</td>';
...
function save(){
	showKPLoadingBar();
	// collect basic data
	var params = ["koolproxyr_mode", "koolproxyr_reboot", "koolproxyr_reboot_hour", "koolproxyr_reboot_min", "koolproxyr_reboot_inter_hour", "koolproxyr_reboot_inter_min", "koolproxyr_acl_method", "koolproxyr_acl_default_mode"];
	var params_chk = ["koolproxyr_enable", "koolproxyr_rule_enable_d1", "koolproxyr_rule_enable_d2", "koolproxyr_rule_enable_d3", "koolproxyr_rule_enable_d4", "koolproxyr_rule_enable_d5", "koolproxyr_rule_enable_d6", "koolproxyr_rule_enable_d7", "koolproxyr_rule_enable_d8"];
	for (var i = 0; i < params.length; i++) {
	...
	for ( var i = 1; i <= maxid; ++i ) {
		if (E("koolproxyr_acl_ip_" + i) || E("koolproxyr_acl_mac_" + i)){
	...
	if(E("koolproxyr_rule_enable_d3").checked == true){
		sourceList += "1|kp.dat|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/kp.dat|视频规则>"
	}else{
		sourceList += "0|kp.dat|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/kp.dat|视频规则>"
	}
	if(E("koolproxyr_rule_enable_d5").checked == true){
		sourceList += "1|easylistchina.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/easylistchina.txt|静态规则>"
	}else{
		sourceList += "0|easylistchina.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/easylistchina.txt|静态规则>"
	}
	if(E("koolproxyr_rule_enable_d6").checked == true){
		sourceList += "1|fanboy-annoyance.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/fanboy-annoyance.txt|静态规则>"
	}else{
		sourceList += "0|fanboy-annoyance.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/fanboy-annoyance.txt|静态规则>"
	}
	if(E("koolproxyr_rule_enable_d7").checked == true){
		sourceList += "1|kpr_video_list.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/kpr_video_list.txt|视频规则>"
	}else{
		sourceList += "0|kpr_video_list.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/kpr_video_list.txt|视频规则>"
	}
	if(E("koolproxyr_rule_enable_d8").checked == true){
		sourceList += "1|yhosts.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/yhosts.txt|视频规则>"
	}else{
		sourceList += "0|yhosts.txt|https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/yhosts.txt|视频规则>"
	}
	maxid = parseInt($("#rule_table > tbody > tr:eq(-2) > td:nth-child(1) > input").attr("id").split("_")[3]);
	for ( var i = 1; i <= maxid; ++i ) {
		if (E("koolproxyr_rule_enable_" + i)){
								...
								<tr>
									<td bgcolor="#4D595D" colspan="3" valign="top" style="border-radius: 8px">
										...
										<div id="vpnc_settings" class="contentM_qis" style="box-shadow: 3px 3px 10px #000;margin-top: -65px;">
											...
											<div style="margin-left:15px"><i>2&nbsp;&nbsp;如果你更改了user.txt，你需要重新重启koolproxyR插件才能让新加入的规则才能生效。</i></div>
												...
												<tr id="auto_reboot_switch">
													...
													<td>
														...
														<span id="koolproxyr_reboot_hour_span">
															&nbsp;&nbsp;&nbsp;&nbsp;
															每天
															<select id="koolproxyr_reboot_hour" name="koolproxyr_reboot_hour" class="ssconfig input_option" >
															</select>
															<select id="koolproxyr_reboot_min" name="koolproxyr_reboot_min" class="ssconfig input_option" >
															</select>
															重启
															&nbsp;&nbsp;&nbsp;&nbsp;
														</span>
														<span id="koolproxyr_reboot_interval_span">
															&nbsp;&nbsp;&nbsp;&nbsp;
															每隔
															<select id="koolproxyr_reboot_inter_hour" name="koolproxyr_reboot_inter_hour" class="ssconfig input_option" >
															</select>
															<select id="koolproxyr_reboot_inter_min" name="koolproxyr_reboot_inter_min" class="ssconfig input_option" >
															</select>
															重启koolproxyR
															&nbsp;&nbsp;&nbsp;&nbsp;
														</span>
										...
										<div id="rule_table_div" style="margin:10px 0px 0px 0px;width:748px">
											<table class="FormTable1" id="rule_table" style="margin:-1px 0px 0px 0px;border: 1px solid #000000;width:100%;" border="1" align="center" cellpadding="9" cellspacing="0" bordercolor="#6b8fa3" >
												<thead>
												<tr>
												...
												<tr>
													...
													<td>
														https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/kp.dat
													</td>
													<td style="text-align:center;">
														视频规则
													</td>
												...
												<tr>
													...
													<td>
														https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/easylistchina.txt
													</td>
												...
												<tr>
													...
													<td>
														https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/fanboy-annoyance.txt
													</td>
													<td style="text-align:center;">
														静态规则
													</td>
												...
												<tr>
													<td style="text-align:center;">
														<input type="checkbox" id="koolproxyr_rule_enable_d7" checked="checked" name="koolproxyr_rule_enable_d7" />
													</td>
													<td>
														kpr_video_list.txt
													</td>
													<td>
														https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/kpr_video_list.txt
													</td>
													<td style="text-align:center;">
														视频规则
													</td>
												...
												<tr>
													<td style="text-align:center;">
														<input type="checkbox" id="koolproxyr_rule_enable_d8" checked="checked" name="koolproxyr_rule_enable_d8" />
													</td>
													<td>
														yhosts.txt
													</td>
													<td>
														https://github.com/user1121114685/koolproxyR/tree/master/koolproxyR/koolproxyR/data/rules/yhosts.txt
													</td>
													<td style="text-align:center;">
														静态规则
													</td>
										...
										<div id="ACL_note" style="margin-top: 5px;">
											...
											<div><i>3&nbsp;&nbsp;如果想在多台装有koolroxyR的路由设备上使用一个证书，请用winscp软件备份/jffs/softcenter/koolproxyr/data文件夹，并上传到另一台路由。</i></div>
										</div>
										<div class="apply_gen">
