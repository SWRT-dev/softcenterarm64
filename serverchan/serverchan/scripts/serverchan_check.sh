...
if [ "$(dbus get softcenter_server_tcode)" == "CN" ]; then
	if [ "$productid" == "BLUECAVE" ];then
		softcenter_app_url="http://update.wifi.com.cn/mips/softcenter/app.json.js"
	else
		softcenter_app_url="http://update.wifi.com.cn/arm/softcenter/app.json.js"
	fi
else
	if [ "$productid" == "BLUECAVE" ];then
		softcenter_app_url="https://sc.paldier.com/softcenter/app.json.js"
	else
		softcenter_app_url="https://scarm.paldier.com/softcenter/app.json.js"
	fi
fi
...
if [[ "${serverchan_info_temp}" == "1" ]]; then
    ...
    echo "#### **CPU温度状况:**" >> ${serverchan_info_text}
