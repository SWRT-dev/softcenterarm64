...
create_ss_json(){
	...
	if [ "$ss_basic_udp2raw_boost_enable" == "1" ] || [ "$ss_basic_udp_boost_enable" == "1" ];then
		if [ "$ss_basic_udp_upstream_mtu" == "1" ] && [ "$ss_basic_udp_node" == "$ssconf_basic_node" ];then
			...
			cat /jffs/softcenter/ss/ss.json | jq --argjson MTU $ss_basic_udp_upstream_mtu_value '. + {MTU: $MTU}' > /jffs/softcenter/ss/ss_tmp.json
			mv /jffs/softcenter/ss/ss_tmp.json /jffs/softcenter/ss/ss.json
		fi
	fi
}
...
start_dns(){
	...
	if [ "$ss_foreign_dns" == "1" ];then
		...
		cdns -c /jffs/softcenter/ss/rules/cdns.json > /dev/null 2>&1 &
	fi
	...
	if [ "$ss_foreign_dns" == "2" ];then
		...
		chinadns -p $DNS_PORT -s $ss_chinadns_user -e $clinet_ip,$ss_real_server_ip -c /jffs/softcenter/ss/rules/chnroute.txt >/dev/null 2>&1 &
	fi
	...
	if [ "$ss_foreign_dns" == "3" ] || [ -z "$ss_foreign_dns" ];then
		...
		if [ "$ss_basic_type" == "4" ]; then
			/jffs/softcenter/bin/trojan -c /jffs/softcenter/ss/trojan2.json >/dev/null 2>&1 &
		fi
		start_sslocal
	...
	if [ "$ss_foreign_dns" == "5" ];then
		...
		chinadns1 -p $DNS_PORT -s $CDN,127.0.0.1:1055 -d -c /jffs/softcenter/ss/rules/chnroute.txt > /dev/null 2>&1 &
	fi
