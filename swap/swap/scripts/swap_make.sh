...
start_make(){
	...
	[ -z "$FORMAT" ] && FORMAT=`echo $swap_make_part_format|grep tntfs`
	[ -z "$FORMAT" ] && FORMAT=`echo $swap_make_part_format|grep tfat`
	if [ -n "$FORMAT" ];then
